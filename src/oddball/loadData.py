...
def get_feature(G):
    ...
    for ite in nodelist:
        ...
        for nei in iNeighbor:
            Wi += G[ite][nei]['weight']
            egonet.add_edge(ite, nei, weight=G[ite][nei]['weight'])
        ...
        for it1 in range(iNeighborLen):
            for it2 in range(it1+1, iNeighborLen):
                ...
                if iNeighbor[it1] in list(G.neighbors(iNeighbor[it2])):
                    ...
                    Wi += G[iNeighbor[it2]][iNeighbor[it1]]['weight']
                    egonet.add_edge(iNeighbor[it2], iNeighbor[it1], weight=G[iNeighbor[it2]][iNeighbor[it1]]['weight'])
        try:
            egonet_adjacency_matrix = nx.adjacency_matrix(egonet).todense()
        except:
            print(ite)
            print(list(G.neighbors(ite)))
            print(egonet)
